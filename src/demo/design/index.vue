...
export default {
  ...
  methods: {
    ...
    getPanel() {
      ...
      // 对所有 panel 进行版本解析
      const panelKeys = Object.keys(panelModules).filter(k => /\/panel-?/.test(k))
    ...
    getVersion(version) {
      ...
      script.addEventListener("load", () => {
        ...
        defaultElementTypeProvider = window.vue3Hiprint ? window['@jake-gao/vue3-hiprint'].defaultElementTypeProvider : window['vue-plugin-hiprint'].defaultElementTypeProvider
        this.init()
      })
      ...
      head.append(
        ...
        $(`<link rel="stylesheet" type="text/css" media="print" href="https://unpkg.com/@jake-gao/vue3-hiprint@${version}/dist/print-lock.css">`)[0]
      )
      head.append(script)
    },
    init() {
